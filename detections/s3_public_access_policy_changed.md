# S3 Bucket Public Access or Policy Changed

Detects changes to S3 public access settings, bucket ACLs, or bucket policies.

## 1. Rule Details

```text
Rule Name: S3 Bucket Public Access or Policy Changed
Severity: High
Risk Score: 67
...
```kql
data_stream.dataset:"aws.cloudtrail" and event.provider:"s3.amazonaws.com" and event.action:("PutBucketPolicy" or "DeleteBucketPolicy" or "PutBucketAcl" or "PutPublicAccessBlock" or "DeletePublicAccessBlock")
```

## 3. CloudTrail Events

```text
PutBucketPolicy, DeleteBucketPolicy, PutBucketAcl, PutPublicAccessBlock, DeletePublicAccessBlock
```

## 4. Why This Matters

S3 permission changes can accidentally or intentionally expose sensitive data.

## 5. Test Procedure

Modify public access block settings on a temporary test bucket, then restore secure settings.
...
![Evidence Screenshot](../screenshots/alert-s3-public-access.png)
